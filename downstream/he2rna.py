...
def calc_corr(omics_mat, omics_gt, gene_list=None):
    corr_list = []
    for i in range(omics_mat.shape[1]):
        corr = np.corrcoef(omics_mat[:, i], omics_gt[:, i])[0, 1]
        corr_list.append(corr)

    import pandas as pd
    if gene_list is None:
        gene_list = [f'Gene_{i}' for i in range(len(corr_list))]
    corr_df = pd.DataFrame(
        {
            'corr': corr_list
        }, index=gene_list
    )
    corr_list_clean = [x for x in corr_list if not np.isnan(x)]
    mean_corr = np.mean(corr_list_clean)
    return mean_corr, corr_df