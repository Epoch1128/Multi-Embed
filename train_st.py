...
def main():
    ...
    parser.add_argument('--index_type', type=str, default='barcode')
    ...
    if args.data_type == 'ST':
        train_data = STDataset(args.image_dir, args.omics_dir, prefixs=data_prefix, index_type=args.index_type)
        ...
        train_data = XeniumDataset(args.image_dir, args.omics_dir, prefixs=data_prefix)
        print(f"length of train data: {len(train_data)}")
        train_loader = DataLoader(train_data, batch_size=args.batch, shuffle=True, num_workers=8, pin_memory=True)
    ...
    train_bar = tqdm(range(0, args.epoch), desc="epoch", total=args.epoch, unit="batch", dynamic_ncols=True)
    for iter in train_bar:
        ...
        if loss_value < loss_min and loss_value < 1.25:
            loss_min = loss_value
            save_checkpoint(model, optimizer, os.path.join(save_dir, f'best_{iter}_{round(loss_value, 2)}.ckpt'))
    
    save_checkpoint(model, optimizer, os.path.join(save_dir, f'last_{iter}_{round(loss_value, 2)}.ckpt'))
...
def train_loop(epoch, model, train_loader, opt, device):
    ...
    for idx, data in enumerate(train_loader):
        ...
        rloss = recon_loss(omic_recon, omic_feat) + recon_loss(img_recon, img_feat) + recon_loss(neg_img_recon, neg_feat)
        rbf_loss = mmd_rbf_loss(omic_emb, img_emb)
        loss_out = rloss * 0.1 + rbf_loss + agg_triple_loss
        # loss_out = triple_loss
        loss_out.backward()
        ...
        loss_sum += loss_value
        bnum += 1
        # train_bar.set_description('epoch:{} iter:{} all:{} recon:{} tri-agg:{}'.format(epoch, idx, round(loss_value, 3), round(rloss.item(), 3), round(agg_triple_loss.item(), 3)))
